...
**Example**:
`Enter L to load a game, or P to play:` <u>L</u>
`Enter the filename:` <u>game1.txt</u>
`
  +---+---+---+---+---+---+---+
  |   |   |   |   |   |   |   |
  +---+---+---+---+---+---+---+
...
`Congratulations <name of winner>! You won!
Draw!`
...
===Enums===
Enums specify a custom primitive type. It gives a name and possible values of the type. Just like a type `bool` has possible values `true` and `false`, in this project we define a type `Result` that has possible values `IllegalMove`, <codeDraw`, `Win`, and `NoResult`. We also define an enum `PieceType` that has possible values `Empty`, `Player1`, and `Player2`.
...
**Examples**
FEN string for an empty board: `7/7/7/7/7/7 x`
...
col 1   2   3   4   5   6   7
`
FEN string for above board: `2x4/7/7/7/7/7/7 o`
...
col 1   2   3   4   5   6   7
`
FEN string for above board: `2x1oo1/2x4/7/7/7/7/7 x`
...
`
If you write:
`int count = piecesInDirection(0, 1, 1, 0);`
...
If you write:
`int count = piecesInDirection(0, 1, 0, 1);`
...
If you write:
`int count = piecesInDirection(0, 0, 1, 0);`
...
If you write:
`int count = piecesInDirection(0, 2, 0, 1);`
